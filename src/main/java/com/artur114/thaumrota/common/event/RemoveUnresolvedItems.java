...
public class RemoveUnresolvedItems { // TODO: 10.05.2025 Переписать!
    public static final String isUUI = ThaumRotA.MODID + "isUseUnresolvedItems";
    public static final String time = ThaumRotA.MODID + "RemoveItemsTime";
    public static final String PRI = ThaumRotA.MODID + "phaseRemoveItems";
        ...
        if (e.player.getEntityData().getBoolean(isUUI)) {
            ...
            if (phaseRemoveItems == 1 && timeRemoveItems >= 20) {
                if (timeRemoveItems == 100 && e.player instanceof EntityPlayerMP) {
                    MiscHandler.sendMessageTranslateWithChangeTitle((EntityPlayerMP) e.player, ThaumRotA.MODID + ".rui.t", rui_title);
                }
                if (timeRemoveItems == 160) {
                    if (e.player instanceof EntityPlayerMP) {
                        MiscHandler.sendMessageTranslateWithChangeTitle((EntityPlayerMP) e.player, ThaumRotA.MODID + ".rui.f", rui_title);
