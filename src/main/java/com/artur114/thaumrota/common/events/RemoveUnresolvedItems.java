...
public class RemoveUnresolvedItems { // TODO: 10.05.2025 Переписать!
    public static final String isUUI = ThaumicRotA.MODID + "isUseUnresolvedItems";
    public static final String time = ThaumicRotA.MODID + "RemoveItemsTime";
    public static final String PRI = ThaumicRotA.MODID + "phaseRemoveItems";
        ...
        if (e.player.getEntityData().getBoolean(isUUI)) {
            ...
            if (phaseRemoveItems == 1 && timeRemoveItems >= 20) {
                if (timeRemoveItems == 100 && e.player instanceof EntityPlayerMP) {
                    MiscHandler.sendMessageTranslateWithChangeTitle((EntityPlayerMP) e.player, ThaumicRotA.MODID + ".rui.t", rui_title);
                }
                if (timeRemoveItems == 160) {
                    if (e.player instanceof EntityPlayerMP) {
                        MiscHandler.sendMessageTranslateWithChangeTitle((EntityPlayerMP) e.player, ThaumicRotA.MODID + ".rui.f", rui_title);
