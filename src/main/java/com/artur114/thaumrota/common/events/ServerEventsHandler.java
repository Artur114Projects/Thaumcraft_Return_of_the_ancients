package com.artur114.thaumrota.common.events;

import com.artur114.thaumrota.common.ancientworld.system.base.AncientLayer1EventsHandler;
import com.artur114.thaumrota.common.events.eventmanagers.PlayerInBiomeManager;
import com.artur114.thaumrota.common.events.eventmanagers.ShortChunkLoadManager;
import com.artur114.thaumrota.common.events.eventmanagers.SlowBuildManager;
import com.artur114.thaumrota.common.events.eventmanagers.TimerTasksManager;
...
public class ServerEventsHandler { // TODO: 10.05.2025 Переписать!
    ...
    public static void playerLoggedInEvent(PlayerEvent.PlayerLoggedInEvent e) {
        if (!e.player.world.isRemote) {
            ...
            if (newVersion) {
                MiscHandler.sendMessageTranslate(player, ThaumicRotA.MODID + ".message.new-version");
                newVersion = false;
            }
        }
    }
    ...
    protected static void checkResearch(EntityPlayer player) {
        ...
        if (AncientPortalsProcessor.hasPortalOnWorld(player.world)) {
            ...
            if (blockPos.distance(player) < 32) {
                MiscHandler.researchAndSendMessage((EntityPlayerMP) player, "m_FOUND_ANCIENT", ThaumicRotA.MODID + ".text.found_portal");
            }
        }
    ...
    private static void tickTimer(EntityPlayer player) {
        ...
        if (timer.hasTimer("recovery")) {
            ...
            if (timer.getTime("recovery") >= 10000) {
                ...
                MiscHandler.researchAndSendMessage((EntityPlayerMP) player, "RECOVERY", ThaumicRotA.MODID + ".text.recovery");
            }
        }
