...
public class BlockProtectEventsHandler {
    ...
    public static void attachCapabilitiesChunk(AttachCapabilitiesEvent<Chunk> e) {
        if (e.getObject() != null && !e.getObject().isEmpty() &&  e.getObject().getWorld().isRemote) CLIENT_MANAGER.attachCapabilitiesEventChunk(e);
        if (e.getObject() != null && !e.getObject().isEmpty() && !e.getObject().getWorld().isRemote) SERVER_MANAGER.attachCapabilitiesEventChunk(e);
    }
